...
function PremiumStatus(props: Props) {
  let [highestPriorityProduct, setHighestPriorityProduct] =
    useState<PremiumProduct>(null);
  ...
  useEffect(() => {
    ...
    if (products.length > 1) {
      for (let i = 1; i < products.length; i++) {
        if (
          Math.abs(
            products[i - 1].expires.getTime() - products[i].expires.getTime()
          ) < 60000
        ) {
          if (
            getPremiumType(products[i - 1]).priority >
            getPremiumType(products[i]).priority
          ) {
            products.splice(i, 1);
          } else {
      ...
      <div>
        {productsToShow?.length > 1 ? (
          <div style={{ overflow: 'hidden' }}>
            <span
              className={styles.premiumStatusLabel}
          ...
          <p>
            ...
            {highestPriorityProduct
              ? getProductListEntry(highestPriorityProduct)
              : 'No Premium'}
          </p>
        )}
      </div>
