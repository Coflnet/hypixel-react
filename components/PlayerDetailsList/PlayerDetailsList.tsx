...
import {
  getHighestPriorityPremiumProduct,
  getPremiumType,
  PREMIUM_RANK,
} from '../../utils/PremiumTypeUtils';
...
function PlayerDetailsList(props: Props) {
  ...
  let [allElementsLoaded, setAllElementsLoaded] = useState(
    props.auctions ? props.auctions.length < FETCH_RESULT_SIZE : false
  );
  ...
  function loadFilters() {
    return api
      .getFilters(filteredItem ? filteredItem.tag : '*')
      .then((filters) => {
        setFilters(filters);
      });
  }
  ...
  function showFilter(): boolean {
    return (
      (props.accountInfo && props.accountInfo?.mcId === props.player.uuid) ||
      (premiumRank && premiumRank.priority > PREMIUM_RANK.STARTER)
    );
  }
  ...
  let loadNewElements = (reset?: boolean): void => {
    ...
    props
      .loadingDataFunction(
        props.player.uuid,
        reset ? 0 : Math.ceil(listElements.length / FETCH_RESULT_SIZE),
        filter
      )
      .then((newListElements) => {
        ...
        listElements = reset
          ? newListElements
          : listElements.concat(newListElements);
        ...
        if (
          listElements.length < FETCH_RESULT_SIZE &&
          newListElements.length !== 0
        ) {
          loadNewElements();
        }
      })
  ...
  let getListState = (): ListState | undefined => {
    return listStates.find((state) => {
      return (
        state.playerUUID === props.player.uuid && state.type === props.type
      );
    });
  };
    ...
    return (
      ...
      >
        <span
          ...
          className={`${styles.disableLinkStyle} ${styles.listItemLink}`}
        >
              ...
              <div>
                <h4>
                  ...
                  {listElement.item.name ||
                    convertTagToName(listElement.item.tag)}
                  {listElement.end.getTime() < Date.now() ||
                  (listElement.bin && listElement.highestBid > 0) ? (
                ...
                <p>
                  Highest Bid:{' '}
                  {numberWithThousandsSeperators(listElement.highestBid)}{' '}
                  {getCoinImage()}
                </p>
                {props.type === 'auctions' ? (
                  <p>
                    Starting Bid:{' '}
                    {numberWithThousandsSeperators(
                      (listElement as Auction).startingBid
                    )}{' '}
                    {getCoinImage()}
                  </p>
                ) : (
                  <p>
                    Highest Own:{' '}
                    {numberWithThousandsSeperators(
                      (listElement as BidForList).highestOwn
                    )}{' '}
                    {getCoinImage()}
                  </p>
                )}
                <p>
                  End of Auction:{' '}
                  {listElement.end.toLocaleTimeString() +
                    ' ' +
                    listElement.end.toLocaleDateString()}
                </p>
              </div>
            </a>
          </Link>
          ...
          <Tooltip
            ...
            tooltipContent={
              <>
                ...
                <p>
                  If you have starter premium or above you are able to use the
                  filter for any player.
                </p>
              </>
            }
          />
