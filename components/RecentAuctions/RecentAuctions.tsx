...
function RecentAuctions(props: Props) {
  let [recentAuctions, setRecentAuctions, recentAuctionsRef] = useStateWithRef<
    RecentAuction[]
  >([]);
  ...
  function loadRecentAuctions(reset: boolean = false) {
    ...
    api
      .getRecentAuctions(props.item.tag, itemFilter)
  ...
  return (
    <div className={styles.recentAuctions}>
      <h3>
        ...
        {!isSSR ? (
          <Form.Control
            as='select'
            defaultValue={
              localStorage.getItem(RECENT_AUCTIONS_FETCH_TYPE_KEY) ||
              RECENT_AUCTIONS_FETCH_TYPE.SOLD
            }
      ...
      {getMoreAuctionsElement(
        ...
        onAfterLogin,
        <span>
          You currently use Starter Premium. You can see up to 120 recent
          auctions with
          <Link href={'/premium'}>
            <a>Premium</a>
          </Link>
