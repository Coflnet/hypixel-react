...
function BazaarPriceGraph(props: Props) {
  ...
  let [chartOptionsPrimary, setChartOptionsPrimary] = useState(
    graphType === GRAPH_TYPE.SINGLE
      ? getPriceGraphConfigSingle()
      : getPriceGraphConfigSplit()
  );
  let [chartOptionsSecondary, setChartOptionsSecondary] = useState(
    getPriceGraphConfigSplit()
  );
  ...
  function init() {
    ...
    graphType =
      (localStorage.getItem(BAZAAR_GRAPH_TYPE) as GRAPH_TYPE) ||
      DEFAULT_GRAPH_TYPE;
      ...
      .then((prices) => {
        if (
          ...
          currentLoadingString !==
            JSON.stringify({ tag: props.item.tag, fetchspan })
        ) {
          return;
        }
        setPrices(prices);
  ...
  function checkForSpecialFetchspanConfiguration() {
    if (fetchspan === DateRange.HOUR) {
      chartOptionsPrimary.legend.data = chartOptionsPrimary.legend.data.filter(
        (s) => !s.includes('Min') && !s.includes('Max')
      );
      chartOptionsSecondary.legend.data =
        chartOptionsSecondary.legend.data.filter(
          (s) => !s.includes('Min') && !s.includes('Max')
        );
      ...
      if (graphType === GRAPH_TYPE.SINGLE) {
        chartOptionsPrimary.legend.data =
          getPriceGraphConfigSingle().legend.data;
        chartOptionsPrimary.series[3].type = 'k';
      } else {
        chartOptionsPrimary.legend.data =
          getPriceGraphConfigSplit().legend.data;
      }
      chartOptionsSecondary.legend.data =
        getPriceGraphConfigSplit().legend.data;
    }
  ...
  function getLegendLocalStorageKey(primary: boolean) {
    return `${graphType}-${primary ? 'primary' : 'secondary'}${
      fetchspan === DateRange.HOUR ? '-hour' : ''
    }`;
  }
  ...
  function onRangeChange(timespan: DateRange) {
    ...
    document.dispatchEvent(
      new CustomEvent(CUSTOM_EVENTS.BAZAAR_SNAPSHOT_UPDATE, {
    ...
    return {
      datazoom: (e) => {
        ...
        let midDate = new Date(
          +chartOptions.xAxis[0].data[
            Math.ceil(chartOptions.xAxis[0].data.length * (mid / 100))
          ]
        );
      ...
      legendselectchanged: (e) => {
        let current = JSON.parse(
          localStorage.getItem(BAZAAR_GRAPH_LEGEND_SELECTION) || '{}'
        );
    ...
    prices.forEach((item, i) => {
      ...
      chartOptionsPrimary.series[0].data.push(
        fetchspan === DateRange.HOUR
          ...
          : [
              item.buyData.price,
              prices[i + 1] ? prices[i + 1].buyData.price : item.buyData.price,
              item.buyData.min,
              item.buyData.max,
            ]
      );
      ...
      if (graphType === GRAPH_TYPE.SINGLE) {
        ...
        chartOptionsPrimary.series[3].data.push(
          item.buyData.volume?.toFixed(2)
        );
        chartOptionsPrimary.series[4].data.push(
          item.buyData.moving?.toFixed(2)
        );
        ...
        chartOptionsPrimary.series[5].data.push(
          fetchspan === DateRange.HOUR
            ...
            : [
                item.sellData.price?.toFixed(2),
                prices[i + 1]
                  ? prices[i + 1].sellData.price?.toFixed(2)
                  : item.sellData.price?.toFixed(2),
        ...
        chartOptionsPrimary.series[8].data.push(
          item.sellData.volume?.toFixed(2)
        );
        chartOptionsPrimary.series[9].data.push(
          item.sellData.moving?.toFixed(2)
        );
      } else {
        chartOptionsPrimary.series[1].data.push(item.buyData.min?.toFixed(2));
        chartOptionsPrimary.series[2].data.push(item.buyData.max?.toFixed(2));
        ...
        chartOptionsSecondary.series[1].data.push(
          item.sellData.min?.toFixed(2)
        );
        chartOptionsSecondary.series[2].data.push(
          item.sellData.max?.toFixed(2)
        );
        chartOptionsSecondary.series[3].data.push(
          item.sellData.volume?.toFixed(2)
        );
        chartOptionsSecondary.series[4].data.push(
          item.sellData.moving?.toFixed(2)
        );
      }
    });
      ...
      <div>
          ...
          {!isSSR ? (
            <>
              <div
                className={
                  graphType === GRAPH_TYPE.SINGLE
                    ? styles.chartWrapperSingle
                    : styles.chartWrapperSplit
                }
              >
                <h3 className={styles.graphHeadline}>
                  {graphType === GRAPH_TYPE.SINGLE ? 'Bazaar data' : 'Buy data'}
                </h3>
                  ...
                  {!isLoading && !noDataFound ? (
                    <ReactECharts
                      ...
                      onEvents={onChartsEvents(
                        ...
                        getLegendLocalStorageKey(false)
                      )}
                    />
                  ) : (
                    graphOverlayElement
        ...
        <div className={styles.additionalInfos}>
          <span className={styles.avgPrice}>
            ...
            {isLoading
              ? '-'
              : numberWithThousandsSeperators(+avgSellPrice.toFixed(1)) +
                ' Coins'}
          </span>
          <span className={styles.avgPrice}>
            <b>Avg Buy Price:</b>{' '}
            {isLoading
              ? '-'
              : numberWithThousandsSeperators(+avgBuyPrice.toFixed(1)) +
                ' Coins'}
          </span>
          <div
            style={{ float: 'left' }}
          ...
          >
            {!isSSR ? (
              <Form.Control
                as='select'
                defaultValue={
                  localStorage.getItem(BAZAAR_GRAPH_TYPE) || DEFAULT_GRAPH_TYPE
                }
          ...
          <div style={{ float: 'right' }}>
            <ShareButton
              ...
              text='Browse the Bazaar history in Hypixel Skyblock'
            />
          </div>
        </div>
        <hr />
