...
function BazaarSnapshot(props: Props) {
  ...
  useEffect(() => {
    ...
    document.addEventListener(
      CUSTOM_EVENTS.BAZAAR_SNAPSHOT_UPDATE,
      onTimestampChangeEvent
    );
  ...
  function loadBazaarSnapshot() {
    bazaarSnapshotDateRef.current =
      typeof debouncedTimestamp?.getTime === 'function'
        ? (debouncedTimestamp as Date).getTime()
        : debouncedTimestamp;

    api
      .getBazaarSnapshot(props.item.tag, debouncedTimestamp)
      .then((snapshot) => {
        if (
          bazaarSnapshotDateRef.current ===
          (typeof debouncedTimestamp?.getTime === 'function'
  ...
  return (
    <>
      <h3 className={styles.headline}>
        {bazaarSnapshot.item.name}Bazaar Snaphot (
        {moment(bazaarSnapshot.timeStamp).format('MMMM Do YYYY, h:mm:ss a')})
      </h3>
      ...
      <div className={styles.flex}>
        ...
        <Card className={styles.informationField}>
          ...
          <Card.Body>
            {getOrderListElement(bazaarSnapshot.sellOrders)}
          </Card.Body>
        </Card>
      </div>
    </>
