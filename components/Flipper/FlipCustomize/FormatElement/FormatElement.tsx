...
function FormatElement(props: Props) {
    ...
    return (
        <div>
            <label htmlFor="modFormat" className="label">
                Custom format
                <Tooltip type="hover" content={<HelpIcon style={{ color: "#007bff", cursor: "pointer" }} />} tooltipContent={formatHelpTooltip} />
            </label>
            <Form.Check onChange={onModDefaultFormatCheckboxChange} defaultChecked={!!props.settings.modFormat} id="modFormat" style={{ display: "inline" }} type="checkbox" />
            {
                props.settings.modFormat ?
                    <div>
                        <div style={{ display: "flex" }}>
                            <Form.Control as="textarea" ref={formatInputRef} style={{ width: "100%" }} onChange={onInputChange} defaultValue={props.settings.modFormat} />
