...
function Search(props: Props) {
  ...
  useEffect(() => {
    if (isClientSideRendering()) {
      setIsSmall(
        isClientSideRendering() ? document.body.clientWidth < 1500 : false
      );
    }
  }, []);
  ...
  let search = () => {
    ...
    searchFunction(searchFor).then((searchResults) => {
      ...
      if (
        ...
        searchFor ===
          (
            (searchElement.current as HTMLDivElement).querySelector(
              '#search-bar'
            ) as HTMLInputElement
          ).value
      ) {
        ...
        let previousSearches: SearchResultItem[] = previousSearchesString
          ? JSON.parse(previousSearchesString)
          : [];
        let matches = 0;
        if (!props.preventDisplayOfPreviousSearches) {
          previousSearches.forEach((prevSearch) => {
            if (
              prevSearch.dataItem.name.indexOf(searchFor) !== -1 &&
              matches < MAX_PREVIOUS_SEARCHES_TO_DISPLAY
            ) {
              prevSearch.isPreviousSearch = true;
              matches++;

              let alreadyFoundIndex = searchResults.findIndex(
                (r) => r.dataItem.name === prevSearch.dataItem.name
              );
  ...
  let onItemClick = (item: SearchResultItem) => {
    ...
    if (
      item.urlSearchParams &&
      new URLSearchParams(window.location.search).toString() !==
        item.urlSearchParams.toString()
    ) {
      setSearchText('');
      setResults([]);
    }
    ...
    let alreadyFoundIndex = previousSearches.findIndex(
      (r) => r.dataItem.name === item.dataItem.name
    );
    if (alreadyFoundIndex === -1) {
  ...
  let searchStyle: React.CSSProperties = {
    ...
    borderLeftWidth: 0,
    borderBottomColor:
      results.length > 0 || noResultsFound ? '#444' : undefined,
  };

  let searchIconStyle: React.CSSProperties = {
    width: isSmall ? 'auto' : '58px',
    borderRadius:
      results.length > 0 || noResultsFound ? '10px 0 0 0' : '10px 0px 0px 10px',
  ...
  function getListItemStyle(i: number): React.CSSProperties {
    let style = {
      backgroundColor:
        i === selectedIndex
          ? props.backgroundColorSelected || '#444'
    ...
    if (results[i]) {
      let isDuplicate =
        results.findIndex(
          (element, index) =>
            element.dataItem.name === results[i].dataItem.name && index !== i
        ) !== -1;
      if (isDuplicate) {
        return {
  ...
  function checkNameChange(uuid: string) {
    api.triggerPlayerNameCheck(uuid).then(() => {
      toast.success(
        'A name check for the player was triggered. This may take a few minutes.'
      );
    });
  }
  ...
  let searchItemContextMenuElement = (
    <div>
      <Menu id={SEARCH_RESULT_CONTEXT_MENU_ID} theme={theme.dark}>
        ...
        >
          <WrongIcon style={{ color: 'red', marginRight: '5px' }} />I didn't
  ...
  return (
    <div
      ...
      style={isSmall ? { marginLeft: '-5px', marginRight: '-5px' } : {}}
    >
            ...
            <InputGroup.Text style={searchIconStyle}>
              {isSmall && !props.hideNavbar ? (
                <div style={{ width: '56px' }}>
                  <NavBar
                    hamburgerIconStyle={{ marginRight: '0px', width: '56px' }}
                  />
                </div>
              ) : (
                <SearchIcon />
        ...
        {noResultsFound
          ...
          : results.map((result, i) => (
              <ListGroup.Item
                ...
                className={
                  result.isPreviousSearch ? styles.previousSearch : null
                }
                onContextMenu={handleSearchContextMenuForSearchResult}
              >
