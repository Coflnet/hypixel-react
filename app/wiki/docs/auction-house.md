---
title: "Auction House Browser"
description: "Navigate and analyze auction house data"
order: 5
---

# Auction House/Bazaar Browser

The Auction House Browser lets you explore over 700 million historical auctions with powerful search and filtering capabilities.  
Via the same search bar you can also search the biggest bazaar database.

## Basic Navigation

### Search
- Use the search bar to find items or players
- Search supports partial names and abbreviations
- Results are ranked by popularity

### Item Pages
Every item has a dedicated page showing:
- **Price graphs** with historical trends both price and items moved
- **Recent auctions** that have ended
- **Active auctions** currently available
- **Related items** for comparison
- **Active orders** the current orderbook for bazaar items

## Price Graphs

### Auction House Items
- **Average Price**: Mean sale price over time
- **Min/Max**: Price range for each time period
- **Volume**: Number of sales per day
- **Mayor Data**: Special events affecting prices

### Bazaar Items
- **Buy/Sell Prices**: Instant buy and sell prices
- **Order Volume**: Number of items traded
- **Moving Average**: Trend indicators
- **Split/Single View**: Choose your preferred layout

### Time Ranges
- **1 Hour**: Real-time bazaar data
- **1 Day**: Recent price movements
- **1 Week**: Short-term trends with candle charts
- **1 Month**: Medium-term analysis
- **All Time**: Complete price history

## Advanced Filtering

### Item Filters
Filter auctions by specific attributes:
- **Enchantments**: Specific enchants and levels
- **Reforges**: Item reforge types
- **Stars**: Dungeon star levels
- **Rarity**: Item tier (Common to Mythic)
- **Price Range**: Minimum and maximum prices
- **Pet Level**: Exact pet level or ranges `1-99` same as `<100`

### Date Filters
- **End Date**: When auctions ended
- **Creation Date**: When items were created

### Player Filters
- **Seller**: Specific player names

## Player Profiles

### Auction History
View a player's:
- **Created Auctions**: Items they've sold or expired
- **Bid History**: Auctions they've bid on
- **Active Auctions**: Current listings
- **Flip Tracking**: Profitable flips made by the player and how high the profit was

### Filtering Player Data
- Filter by item type
- Date range selection
- Price thresholds
- Auction status (sold/unsold)

* player history filtering is a [premium](/premium) feature

## Auction Details

Each auction page shows:
- **Item Information**: Name, tier, category
- **Auction Data**: Start/end times, bids
- **Seller Information**: Player details
- **Bid History**: All bids with timestamps
- **Item Properties**: Enchantments, reforges, NBT data

### Special Features
- **Compare to Similar**: See reference auctions used to determine the item value
- **Copy Commands**: Get in-game auction links for active auctions
- **Subscribe**: Get notifications for similar items (Click the bell icon)
- **Previous Owners**: Track item trading history


## Data Export

### Premium+ Features
- Export historic auction data to CSV
- Custom date ranges
- Advanced filtering options
- Discord webhook delivery when export is done

### Use Cases
- Market research
- Price modeling
- Trading strategies
- Academic analysis

## Tips and Tricks

### Efficient Browsing
1. Use bookmarks for frequently checked items (right click the search result to pin it)
2. Set up notifications instead of manual checking
3. Use the related items feature for discovery

### Market Research
1. Check multiple time ranges for context
2. Compare weekday vs weekend prices to discover priceswings you can profit from
3. Look for update-related price changes
4. Monitor high-volume items for stability (we have tools to notify you as well)

### Finding Opportunities
1. Look for items with high price variance
2. Check low-supply items for manipulation, or just check the [low supply items](/low-supply-items) page
3. Monitor new items for early opportunities
4. Track seasonal items for timing

## Integration with Other Features

### Flipper Integration
- Auction browser helps verify flip estimates
- Use historical data to validate opportunities

### Mod Integration
- Price estimates appear in item tooltips