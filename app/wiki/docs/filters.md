...
## Color
- **Type:** `Equal` with a custom interactive editor.
- **Description:** Exact hex or RGB, lists, summed absolute RGB distance, and six-position repeating patterns. `pattern:ABCABC` matches `D07D07`; `F2DF11-11` matches `F0DE09` using absolute RGB distance, not perceptual Delta E.

See the [complete Color filter guide and interactive builder](/wiki/color-filters) for all modes, matching swatches, pattern presets, validation rules, and default-color behavior in flip groups.

## ExoticColor
- **Type:** `Equal`
- **Description:** Select a color classification, including exotic, original, fairy, crystal, undyed, glitched, and spook. See [all classification options](/wiki/color-filters#other-color-related-filter-options).

## FairyColor
- **Type:** `Equal`
- **Description:** The `Fairy` option matches plain and OG fairy classifications.

## CrystalColor
- **Type:** `Equal`
- **Description:** The `Crystal` option matches the crystal classification.

## HexColorList
- **Type:** `RANGE` with the Color editor.
- **Description:** Select a comma- or space-separated list of exact hex colors, or one pattern or RGB-distance rule. See the [Color filter guide](/wiki/color-filters).
