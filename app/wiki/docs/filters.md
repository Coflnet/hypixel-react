...
## RemoveAfter
Removes this filter group from your settings after the given date and time. The value is a date, so this is useful for temporary event, mayor, or market-condition rules that should clean themselves up.
...
## SellerIsYoutuber
Matches flips whose seller is in SkyCofl's current known-YouTuber list. Use `SellerIsYoutuber=Yes` in a flip filter group.
...
## ExoticColor
- **Type:** `Equal`
- **Description:** Classifies an item's natural armor color as exotic, original, fairy, crystal, undyed, glitched, or Spook. Items colored by a dye are excluded.
- **Current options:** `Any`, `Exotic`, `Original`, `Fairy`, `Fairy+Crystal`, `Crystal`, `Plain Fairy`, `OG Fairy`, `Undyed`, `Glitched`, `Spook`

### How to filter for exotics

On the [flipper page](/flipper), add the **Exotic color** filter and choose **Exotic**. In typed mod filters or API/item links, the equivalent is `ExoticColor=Exotic`; for example, [view exotic Young Dragon Chestplates](/item/YOUNG_DRAGON_CHESTPLATE?ExoticColor=Exotic).

- `Exotic` selects special non-standard colors, excluding known fairy, crystal, original, and undyed colors.
- `Any` selects the broader special-color group used by the filter engine: exotic, glitched, and Spook colors.
- `Fairy` includes plain and OG fairy colors; `Fairy+Crystal` includes both fairy sets and crystal colors.
- `Original` selects the item's normal default color. `Undyed` selects the legacy undyed leather color.
- `Plain Fairy`, `OG Fairy`, `Crystal`, `Glitched`, and `Spook` select only that classification.

The UI only offers this filter for items that have color data. Use the live [`filter/options` endpoint](/api/filter/options?itemTag=*) to check current filter names and values; use a concrete item tag instead of `*` to see only filters applicable to that item.

## FairyColor
- **Type:** `Equal`
- **Description:** Legacy convenience filter for fairy colors. Prefer `ExoticColor=Fairy` for new filters.

## CrystalColor
- **Type:** `Equal`
- **Description:** Legacy convenience filter for crystal colors. Prefer `ExoticColor=Crystal` for new filters.
...
## CakeSoulColor
- **Type:** `Equal`
- **Description:** Color of a Cake Soul, derived from its durability value
- **Options:** `Black`, `Red`, `Dark Green`, `Brown`, `Blue`, `Purple`, `Cyan`, `Light Gray`, `Gray`, `Pink`, `Lime`, `Yellow`, `Light Blue`, `Magenta`, `Orange`, `White`
...
## HasCreationTime
- **Type:** `BOOLEAN`
- **Description:** Whether the item has a creation time set. Use `true` or `false`.
...
## InteligenceEarned
- **Type:** `NUMERICAL, RANGE`
- **Description:** Intelligence earned on a Wizard Wand. The filter name intentionally uses the current API spelling `InteligenceEarned`.

## PickonimbusDurability
- **Type:** `NUMERICAL, RANGE`
- **Description:** Remaining durability on a Pickonimbus Pickaxe.

## ChimeraFound
- **Type:** `NUMERICAL, RANGE`
- **Description:** Number of Ultimate Chimera books found with Diana's Bookshelf.

## GrowthStages
- **Type:** `RANGE`
- **Description:** Growth stage of a Rosewater Flask. Stages run from 0 through 6 and each stage represents 50 hours held; numeric comparisons and ranges are supported.
