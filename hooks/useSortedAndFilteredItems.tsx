...
async function processInChunks<T>(items: T[], processor: (item: T) => boolean, chunkSize: number = 100): Promise<T[]> {
    const result: T[] = []
...
export function useSortedAndFilteredItems<T>(
    ...
    useEffect(() => {
        processItems(items, orderBy, debouncedNameFilter, debouncedMinimumProfit, filterFunction, sortFunctionArgs)
