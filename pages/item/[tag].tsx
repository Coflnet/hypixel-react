...
function ItemDetails(props: Props) {
  ...
  let [item, setItem] = useState<Item>(
    props.item ? parseItem(props.item) : null
  );
  let [prices] = useState<ItemPrice[]>(
    props.prices ? props.prices.map(parseItemPrice) : []
  );
  let [filter] = useState<ItemFilter>(
    props.filter ? parseItemFilter(props.filter) : null
  );
  ...
  return (
    <div className='page'>
      {getHeadElement(
        ...
        `💰 Price: ${
          avgPrice ? numberWithThousandsSeperators(Math.round(avgPrice)) : '---'
        } Coins
        ...
        `${
          getItem().name || convertTagToName(tag)
        } price | Hypixel SkyBlock AH history tracker`
      )}
      <Container>
        <Search selected={getItem()} type='item' />
        {item.bazaar ? (
          <BazaarPriceGraph item={getItem()} />
        ) : (
          <AuctionHousePriceGraph item={getItem()} />
        )}
      </Container>
    </div>
  );
  ...
  let apiResponses = await Promise.all([
    ...
    api
      .getItemPrices(
        params.tag,
        range,
        query.itemFilter ? JSON.parse(atobUnicode(query.itemFilter)) : {}
      )
      .catch(() => {
        return [];
      }),
