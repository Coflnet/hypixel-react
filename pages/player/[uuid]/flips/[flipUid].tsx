...
function Flipper(props: Props) {
  let flipTrackingResponse = parseFlipTrackingResponse(
    props.flipTrackingResponse
  );
  let player = parsePlayer(props.player);
  let targetFlip = props.targetFlip
    ? parseFlipTrackingFlip(props.targetFlip)
    : null;

  function getTargetFlipEmbedDescription(targetFlip: FlipTrackingFlip) {
    return `${
      targetFlip.profit > 0 ? '📈 Profit' : '📉 Loss'
    }:  ${numberWithThousandsSeperators(targetFlip.profit)} Coins ${
      targetFlip.profit > 0
        ? `(${Math.round((targetFlip.profit / targetFlip.pricePaid) * 98)}%)`
        : ''
    }
        💸 Purchase: ${numberWithThousandsSeperators(
          targetFlip.pricePaid
        )} Coins
        💰 Sold: ${numberWithThousandsSeperators(targetFlip.soldFor)} Coins
        🕑 Sold at ${moment(targetFlip.sellTime).format(
    ...
    <div className='page'>
      ...
      <Container>
        <Search
          ...
          currentElement={
            <p>
              <span style={{ fontSize: 'larger', marginRight: '20px' }}>
                Tracked flips of:
              </span>
...
export const getServerSideProps = async ({ res, params }) => {
  ...
  let apiResponses = await Promise.all(
    [
      api.getPlayerName(params.uuid),
      api.getTrackedFlipsForPlayer(params.uuid),
  ...
  return {
    props: {
      ...
      targetFlip:
        (apiResponses[1] as FlipTrackingResponse)?.flips?.find(
          (f) => f.uId.toString(16) === params.flipUid
        ) || null,
    },
  };
};
