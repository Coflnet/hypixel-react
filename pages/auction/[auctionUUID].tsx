...
function AuctionDetailsPage(props: Props) {
  ...
  let [auctionDetails, setAuctionDetails] = useState(
    props.auctionDetails ? parseAuctionDetails(props.auctionDetails) : undefined
  );
  ...
  function getAuctionDescription(): string {
    ...
    description += `Auction for ${auctionDetails?.auction?.item?.name?.replaceAll(
      /§./g,
      ''
    )} by ${auctionDetails?.auctioneer?.name}`;
    if (!auctionDetails.auction.bin) {
      description += `| Highest Bid: ${numberWithThousandsSeperators(
        auctionDetails.auction.highestBid
      )} Coins with ${auctionDetails.bids.length} Bids`;
    } else if (auctionDetails.bids.length > 0) {
      description += `| Bought by ${
        auctionDetails.bids[0].bidder.name
      } for  ${numberWithThousandsSeperators(
        auctionDetails.auction.highestBid
      )} Coins`;
    }

    if (auctionDetails.auction.end > new Date()) {
      description += ` | Ends on ${moment(auctionDetails.auction.end).format(
        'MMMM Do YYYY, h:mm:ss a'
      )}`;
    } else {
      description += ` | Ended on ${moment(auctionDetails.auction.end).format(
        'MMMM Do YYYY, h:mm:ss a'
      )}`;
    }
  ...
  return (
    <div className='page'>
      {auctionDetails
        ? getHeadElement(
            `Auction for ${auctionDetails?.auction?.item?.name?.replaceAll(
              /§./g,
              ''
            )} by ${
            ...
            [
              auctionDetails.auction.item.name ||
                auctionDetails.auction.item.tag,
              auctionDetails.auctioneer.name,
            ],
