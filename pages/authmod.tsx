...
function AuthMod() {
  ...
  function authenticateModConnection() {
    ...
    api
      .authenticateModConnection(conId)
      .then(() => {
        ...
        {!isSSR ? (
          <div>
            {isLoggedIn && isAuthenticated && conId !== null ? (
              <div>
                <p style={{ color: '#40ff00' }}>
                  Your Connection is now authorized
                </p>
                <hr />
                <p>
                  Now that the mod is connected with the website you can change
                  the filters/settings here:
                </p>
                <Link href='/flipper'>
            ...
            conId !== null
              ...
              : null}
            {!isLoggedIn && !hasAuthenticationFailed && conId !== null ? (
              <p>Please log in to authenticate for the mod usage</p>
            ) : null}
            {hasAuthenticationFailed ? (
              <p>
                Authentication failed.{' '}
                <Button onClick={authenticateModConnection}>Try again</Button>
              </p>
            ) : null}
            {conId === null ? (
              <p>This is an invalid link. There is no connection id present.</p>
            ) : null}
          </div>
        ) : null}
