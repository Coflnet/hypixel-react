...
test('open item with sharpness 5', async ({ page, isMobile }) => {
    ...
    await page.locator('form').filter({ hasText: 'SharpnessNone1234567' }).getByRole('textbox').focus()
    await page.locator('form').filter({ hasText: 'SharpnessNone1234567' }).getByRole('textbox').fill('5')
