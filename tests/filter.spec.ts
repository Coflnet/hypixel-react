...
test('open item with sharpness 5', async ({ page, isMobile }) => {
    ...
    await page.locator('form').filter({ hasText: 'SharpnessNone1234567Please fill the filter or remove it' }).getByRole('textbox').focus()
    await page.locator('form').filter({ hasText: 'SharpnessNone1234567Please fill the filter or remove it' }).getByRole('textbox').fill('5')
