...
function Search(props: Props) {
    ...
    let getSelectedElement = (): JSX.Element => {
        ...
        return <p className="current"><Badge variant="primary">Current:</Badge> <img crossOrigin="anonymous" src={props.selected.iconUrl} width="32" height="32" alt="item icon" style={{ marginRight: "10px" }} loading="lazy"/>{props.selected.name || convertTagToName((props.selected as Item).tag)}</p>
    }
                                ...
                                results.map((result, i) => (
                                    <ListGroup.Item key={result.id} action onClick={(e: any) => { onItemClick(result) }} style={i === results.length - 1 ? { marginBottom: "10px" } : {}} >
                                        {result.dataItem.iconUrl ?
                                            <img className="search-result-icon" crossOrigin="anonymous" width={32} height={32} src={result.dataItem.iconUrl} alt="item icon" loading="lazy"/> :
                                            <Spinner animation="border" role="status" variant="primary" />
                                        }
