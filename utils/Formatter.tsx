...
export function enchantmentAndReforgeCompare(
  a: Enchantment | Reforge,
  b: Enchantment | Reforge
): number {
...
export function formatToPriceToShorten(
  num: number,
  decimals: number = 0
): string {
  ...
  if (num >= 1_000_000_000)
    return checkPreviousNegativeSymbol(
      (num / 1_000_000_000).toFixed(decimals) + 'B'
    );
  if (num >= 1_000_000)
    return checkPreviousNegativeSymbol(
      (num / 1_000_000).toFixed(decimals) + 'M'
    );
  if (num >= 1_000)
    return checkPreviousNegativeSymbol((num / 1_000).toFixed(decimals) + 'k');
 ...
 */
export function getNumberFromShortenString(
  shortString?: string
): number | undefined {
  if (!shortString) {
    return;
...
export function formatDungeonStarsInString(
  stringWithStars: string,
  style: CSSProperties = {},
  dungeonItemLevelString?: string
): JSX.Element {
  let yellowStarStyle = {
    color: 'yellow',
    fontWeight: 'normal',
  ...
  if (!starWithNumber && numberOfMasterstars) {
    ...
    for (
      let index = 0;
      index < starsString.length - numberOfMasterstars;
      index++
    ) {
      yellowStarsString += '✪';
    }
  ...
  if (starWithNumber) {
    normalStarElement = (
      <span style={yellowStarStyle}>
        {starsString.substring(0, starsString.length - 1)}
      </span>
    );
  }
...
export function getMinecraftColorCodedElement(
  text: string,
  autoFormat = true
): JSX.Element {
  ...
  splits.forEach((split, i) => {
    ...
    let text = autoFormat
      ? convertTagToName(split.substring(1, split.length))
